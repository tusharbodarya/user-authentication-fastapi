...
from config import utils, oauth2
...
# register user
@router.post('/register', status_code=status.HTTP_201_CREATED, response_model=users.UserResponse)
...
# login user
@router.post('/login')
def login(payload: users.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
